...
class CatalogIntegrityTests(unittest.TestCase):
    def test_catalog_contains_exactly_all_canonical_ids(self):
        ...
        self.assertEqual(len(catalog_ids), 1771)
...
class CoverageAndFamilyTests(unittest.TestCase):
    ...
    def test_no_alias_double_count(self):
        ...
        self.assertEqual(len({entry.fact_id for entry in report.entries}), 1771)
...
class SeedApprovedRawTests(unittest.TestCase):
    def test_seeded_approved_raw_ids(self):
        ...
        self.assertEqual(len(APPROVED_RAW_FACT_IDS), 476)
...
class SagittariusFamilyS44BTests(unittest.TestCase):
    ...
    def test_global_totals_after_s418b(self):
        report = build_human_copy_catalog()
        self.assertEqual(report.total_facts, 1771)
        self.assertEqual(report.approved_override_count, 675)
        self.assertEqual(report.approved_raw_count, 476)
        self.assertEqual(report.needs_review_count, 2)
        self.assertEqual(report.unreviewed_count, 618)
        self.assertEqual(
            report.approved_override_count
            + report.approved_raw_count
            + report.needs_review_count
        ...
        self.assertEqual(report.reviewed_count, 1153)
        self.assertEqual(report.presentation_ready_count, 1151)
...
class SignReviewQueueS46Tests(unittest.TestCase):
    ...
    def test_queue_does_not_mutate_registries_or_totals(self):
        ...
        self.assertEqual(catalog.total_facts, 1771)
...
class CancerVirgoFamilyS411BTests(unittest.TestCase):
    ...
    def test_s411b_registries_and_wording_corrections(self):
        ...
        self.assertEqual(len(NEEDS_REVIEW_FACT_IDS), 2)
        self.assertEqual(len(HUMAN_COPY_OVERRIDES), 675)
        self.assertEqual(len(APPROVED_RAW_FACT_IDS), 476)
        self.assertTrue(
            set(HUMAN_COPY_OVERRIDES).isdisjoint(APPROVED_RAW_FACT_IDS)
        )
...
class MotionRetrogradeS414BTests(unittest.TestCase):
    ...
    def test_s414b_registries_and_wording(self):
        ...
        self.assertEqual(by_type["aspect"], 618)
...
class House1S415BTests(unittest.TestCase):
    """S4.15B: complete house:1 human-copy layer (16 new overrides)."""

    FROZEN_OVERRIDE = (
        "h1_support_intellectual_work",
        "May support intellectual work and transport-related professions.",
    )
    S415B_OVERRIDES: dict[str, str] = {
    ...
    def test_house_1_bio_copy_frozen_and_family_complete(self):
        report = build_human_copy_catalog()
        ...
        self.assertEqual(house_tot, 383)
        self.assertEqual(house_rev, 383)
        self.assertEqual(house_ready, 382)
        self.assertEqual(house_ur, 0)

    def test_s415b_overrides_and_semantics(self):
        by_id = {fact.id: fact for fact in ALL_SOURCE_FACTS}
        frozen_id, frozen_human = self.FROZEN_OVERRIDE
        self.assertEqual(HUMAN_COPY_OVERRIDES[frozen_id], frozen_human)
        self.assertEqual(len(self.S415B_OVERRIDES), 16)
        for fact_id, human in self.S415B_OVERRIDES.items():
            with self.subTest(override=fact_id):
                self.assertEqual(HUMAN_COPY_OVERRIDES[fact_id], human)
                self.assertNotIn(fact_id, APPROVED_RAW_FACT_IDS)
                self.assertNotIn(fact_id, NEEDS_REVIEW_FACT_IDS)
                ...
                self.assertEqual(entry.canonical_text, by_id[fact_id].text)
                self.assertNotEqual(entry.canonical_text, human)
        amplify = HUMAN_COPY_OVERRIDES["h1_strengthens_mercury_functions"]
        self.assertEqual(
            amplify,
            "May amplify Mercury-related patterns in thinking, communication, "
            "and learning.",
        )
        self.assertIn("amplify", amplify.lower())
        self.assertNotIn("improve", amplify.lower())
        self.assertNotIn("strengthen thinking", amplify.lower())
        self.assertNotIn(
            "Mercury functions",
            amplify,
        )
...
class House1S433BTests(unittest.TestCase):
    """S4.33B: House 1 Lesson 7 human-copy delta (22 overrides + 5 raw)."""

    S433B_APPROVED_RAW: tuple[str, ...] = (
    ...
    FROZEN_BIO_OVERRIDES: dict[str, str] = {
        "h1_support_intellectual_work": (
            "May support intellectual work and transport-related professions."
        ),
        "h1_eventfulness_books": (
    ...
    def test_house_1_l7_delta_reviewed_and_ready(self):
        report = build_human_copy_catalog()
        family = next(f for f in report.families if f.family_key == "house:1")
        self.assertEqual(family.total_facts, 44)
        self.assertEqual(family.approved_override, 39)
        ...
        self.assertEqual(family.reviewed_count, 44)
        self.assertEqual(family.presentation_ready_count, 44)
        self.assertEqual(len(self.S433B_OVERRIDES), 22)
        self.assertEqual(len(self.S433B_APPROVED_RAW), 5)

    def test_s433b_overrides_raw_and_frozen_bio(self):
        by_id = {fact.id: fact for fact in ALL_SOURCE_FACTS}
        self.assertEqual(len(self.FROZEN_BIO_OVERRIDES), 17)
        for fact_id, human in self.FROZEN_BIO_OVERRIDES.items():
            with self.subTest(frozen_bio=fact_id):
                self.assertEqual(HUMAN_COPY_OVERRIDES[fact_id], human)
                self.assertNotIn(fact_id, APPROVED_RAW_FACT_IDS)
                self.assertNotIn(fact_id, NEEDS_REVIEW_FACT_IDS)
        for fact_id in self.S433B_APPROVED_RAW:
            with self.subTest(approved_raw=fact_id):
                self.assertIn(fact_id, APPROVED_RAW_FACT_IDS)
                self.assertNotIn(fact_id, HUMAN_COPY_OVERRIDES)
                self.assertNotIn(fact_id, NEEDS_REVIEW_FACT_IDS)
                entry = build_catalog_entry(by_id[fact_id])
                ...
                self.assertEqual(entry.human_text, entry.canonical_text)
        for fact_id, human in self.S433B_OVERRIDES.items():
            with self.subTest(override=fact_id):
                self.assertEqual(HUMAN_COPY_OVERRIDES[fact_id], human)
                self.assertNotIn(fact_id, APPROVED_RAW_FACT_IDS)
                self.assertNotIn(fact_id, NEEDS_REVIEW_FACT_IDS)
    ...
    def test_s433b_semantic_guardrails(self):
        teaching = HUMAN_COPY_OVERRIDES["h1_l7_teaching_association"]
        commerce = HUMAN_COPY_OVERRIDES["h1_l7_commerce_association"]
        accounting = HUMAN_COPY_OVERRIDES["h1_l7_accounting_association"]
        scientist = HUMAN_COPY_OVERRIDES["h1_l7_scientist_association"]
        mathematician = HUMAN_COPY_OVERRIDES["h1_l7_mathematician_association"]
        for copy in (teaching, commerce, accounting, scientist, mathematician):
            with self.subTest(occupation=copy):
                lowered = copy.lower()
                self.assertIn("favorable association", lowered)
                self.assertNotIn("talent", lowered)
                self.assertNotIn("competence", lowered)
                self.assertNotIn("certif", lowered)
                self.assertNotIn("success", lowered)
                self.assertNotIn("hir", lowered)
                self.assertNotIn("ability", lowered)
        self.assertNotIn("sales", commerce.lower())
        self.assertIn("commercial", commerce.lower())
        bodily = HUMAN_COPY_OVERRIDES["h1_l7_bodily_mobility"]
        self.assertIn("physically mobile", bodily.lower())
        self.assertNotIn("travel", bodily.lower())
        self.assertNotIn("trip", bodily.lower())
        witty = HUMAN_COPY_OVERRIDES["h1_l7_quick_wittedness"]
        self.assertIn("quick-witted", witty.lower())
        self.assertNotIn("fast thinking", witty.lower())
        self.assertNotEqual(
            HUMAN_COPY_OVERRIDES["h1_l7_talkativeness"],
            HUMAN_COPY_OVERRIDES["h1_l7_excessive_talkativeness"],
        )
        self.assertNotEqual(
            HUMAN_COPY_OVERRIDES["h1_l7_fast_movements"],
            witty,
        )
        self.assertIn("movements", HUMAN_COPY_OVERRIDES["h1_l7_fast_movements"].lower())
        self.assertIn("movements", HUMAN_COPY_OVERRIDES["h1_l7_precise_movements"].lower())
        self.assertNotIn(
            "h1_l7_starts_but_does_not_complete_tasks", HUMAN_COPY_OVERRIDES
        )
        two_faced = HUMAN_COPY_OVERRIDES["h1_l7_two_facedness"]
        self.assertIn("two-faced", two_faced.lower())
        self.assertIn("duplicitous", two_faced.lower())
        self.assertTrue(two_faced.lower().startswith("may "))


class House2S416BTests(unittest.TestCase):
    """S4.16B: complete house:2 human-copy layer (19 overrides + 1 raw)."""

    S416B_APPROVED_RAW = "h2_profit_through_public_speaking"
    S416B_OVERRIDES: dict[str, str] = {
    ...
    def test_house_2_l7_copy_frozen_and_bio_unreviewed(self):
        ...
        self.assertEqual(family.approved_override, 21)
        self.assertEqual(family.approved_raw, 7)
        self.assertEqual(family.needs_review, 0)
        self.assertEqual(family.unreviewed, 0)
    ...
    def test_s416b_registries_and_semantics(self):
        by_id = {fact.id: fact for fact in ALL_SOURCE_FACTS}
        self.assertEqual(len(self.S416B_OVERRIDES), 19)
        self.assertIn(self.S416B_APPROVED_RAW, APPROVED_RAW_FACT_IDS)
        self.assertNotIn(self.S416B_APPROVED_RAW, HUMAN_COPY_OVERRIDES)
        raw_entry = build_catalog_entry(by_id[self.S416B_APPROVED_RAW])
        self.assertEqual(raw_entry.review_status, STATUS_APPROVED_RAW)
        self.assertEqual(
            raw_entry.human_text,
            "Profit may come through public speaking.",
        )
        self.assertEqual(raw_entry.human_text, raw_entry.canonical_text)
        for fact_id, human in self.S416B_OVERRIDES.items():
            with self.subTest(override=fact_id):
                self.assertEqual(HUMAN_COPY_OVERRIDES[fact_id], human)
                self.assertNotIn(fact_id, APPROVED_RAW_FACT_IDS)
                self.assertNotIn(fact_id, NEEDS_REVIEW_FACT_IDS)
                ...
                self.assertEqual(entry.canonical_text, by_id[fact_id].text)
                self.assertNotEqual(entry.canonical_text, human)
        mech = HUMAN_COPY_OVERRIDES["h2_financial_mechanism_design"]
        scheme = HUMAN_COPY_OVERRIDES["h2_financial_scheme_creation"]
        self.assertNotEqual(mech, scheme)
        self.assertIn("mechanisms", mech)
        self.assertIn("schemes", scheme)
        self.assertNotIn("guaranteed", mech.lower())
        self.assertNotIn("guaranteed", scheme.lower())
        self.assertNotIn("literacy", mech.lower())
        self.assertNotIn("investment", mech.lower())
        app = HUMAN_COPY_OVERRIDES["h2_objects_attract_through_applicability"]
        interest = HUMAN_COPY_OVERRIDES["h2_objects_attract_through_interestingness"]
        useful = HUMAN_COPY_OVERRIDES["h2_objects_attract_through_usefulness"]
        self.assertNotEqual(app, interest)
...
class House3S417BTests(unittest.TestCase):
    """S4.17B: complete house:3 human-copy layer (21 overrides + 1 raw)."""

    S417B_APPROVED_RAW = "h3_reads_a_lot"
    CONCENTRATION_ID = "h3_intellectual_success_depends_on_concentration"
    CONCENTRATION_HUMAN = (
        "Intellectual success may depend on the ability to maintain "
        "concentration."
    )
    S417B_OVERRIDES: dict[str, str] = {
    ...
    def test_house_3_l7_copy_frozen_and_bio_unreviewed(self):
        report = build_human_copy_catalog()
        ...
        self.assertEqual(house1.reviewed_count, 44)
        self.assertEqual(house1.unreviewed, 0)
        self.assertEqual(house2.reviewed_count, 28)
        ...
        for house_n in range(5, 13):
            with self.subTest(untouched=house_n):
                ...
                self.assertEqual(fam.unreviewed, 0)
                self.assertEqual(fam.reviewed_count, fam.total_facts)

    def test_s417b_registries_and_semantics(self):
        by_id = {fact.id: fact for fact in ALL_SOURCE_FACTS}
        self.assertEqual(len(self.S417B_OVERRIDES), 21)
        self.assertIn(self.S417B_APPROVED_RAW, APPROVED_RAW_FACT_IDS)
        self.assertNotIn(self.S417B_APPROVED_RAW, HUMAN_COPY_OVERRIDES)
        raw_entry = build_catalog_entry(by_id[self.S417B_APPROVED_RAW])
        self.assertEqual(raw_entry.review_status, STATUS_APPROVED_RAW)
        self.assertEqual(raw_entry.human_text, "Reads a lot.")
        self.assertEqual(raw_entry.human_text, raw_entry.canonical_text)
        for fact_id, human in self.S417B_OVERRIDES.items():
            with self.subTest(override=fact_id):
                self.assertEqual(HUMAN_COPY_OVERRIDES[fact_id], human)
                self.assertNotIn(fact_id, APPROVED_RAW_FACT_IDS)
                self.assertNotIn(fact_id, NEEDS_REVIEW_FACT_IDS)
                ...
                self.assertEqual(entry.canonical_text, by_id[fact_id].text)
                self.assertNotEqual(entry.canonical_text, human)
        concentration = build_catalog_entry(by_id[self.CONCENTRATION_ID])
        self.assertEqual(concentration.review_status, STATUS_APPROVED_OVERRIDE)
        self.assertNotEqual(concentration.review_status, STATUS_NEEDS_REVIEW)
        self.assertNotIn(self.CONCENTRATION_ID, NEEDS_REVIEW_FACT_IDS)
        self.assertEqual(concentration.human_text, self.CONCENTRATION_HUMAN)
        self.assertEqual(
            HUMAN_COPY_OVERRIDES[self.CONCENTRATION_ID],
            self.CONCENTRATION_HUMAN,
        )
        self.assertTrue(by_id[self.CONCENTRATION_ID].unresolved)
        questions = HUMAN_COPY_OVERRIDES["h3_ability_to_ask_right_questions"]
        self.assertNotIn("interview", questions.lower())
        self.assertNotIn("persuasion", questions.lower())
        attention = HUMAN_COPY_OVERRIDES["h3_ability_to_distribute_attention"]
        self.assertNotIn("multitask", attention.lower())
        self.assertNotIn("adhd", attention.lower())
        tactical = HUMAN_COPY_OVERRIDES["h3_ability_to_solve_tactical_tasks"]
        self.assertIn("tactical", tactical.lower())
        self.assertNotIn("strateg", tactical.lower())
        switching = HUMAN_COPY_OVERRIDES["h3_ability_to_switch_between_activities"]
        self.assertNotIn("adaptab", switching.lower())
        arguments = HUMAN_COPY_OVERRIDES["h3_arguments_readily_available"]
        self.assertNotIn("debate", arguments.lower())
        self.assertNotIn("persuasion", arguments.lower())
        curiosity = HUMAN_COPY_OVERRIDES["h3_extreme_curiosity"]
        self.assertNotIn("analy", curiosity.lower())
        group = HUMAN_COPY_OVERRIDES["h3_group_learning_easier"]
        self.assertNotIn("teamwork", group.lower())
        fly = HUMAN_COPY_OVERRIDES["h3_knowledge_grasped_on_the_fly"]
        self.assertNotIn("intelligence", fly.lower())
        langs = HUMAN_COPY_OVERRIDES["h3_learns_languages"]
        self.assertNotIn("certified", langs.lower())
        contacts = HUMAN_COPY_OVERRIDES["h3_many_unnecessary_contacts"]
        self.assertNotIn("network", contacts.lower())
        self.assertNotIn("patholog", contacts.lower())
        story = HUMAN_COPY_OVERRIDES["h3_skilled_storyteller"]
        self.assertNotIn("persuasion", story.lower())
        circle = HUMAN_COPY_OVERRIDES["h3_wide_circle_of_acquaintances"]
        self.assertNotIn("network", circle.lower())
        essays = HUMAN_COPY_OVERRIDES["h3_writes_essays_well"]
        self.assertNotIn("aptitude", essays.lower())
    ...
    def test_no_concentration_resolver_or_proxy(self):
        import ast
        import pathlib
...
class House4S418BTests(unittest.TestCase):
    ...
    def test_house_4_l7_copy_frozen_and_bio_unreviewed(self):
        report = build_human_copy_catalog()
        ...
        self.assertEqual(family.review_coverage, 1.0)
        self.assertEqual(family.presentation_ready_coverage, round(20 / 21, 6))
        ...
        self.assertEqual(house2.reviewed_count, 28)
        self.assertEqual(house2.unreviewed, 0)
        self.assertEqual(house3.reviewed_count, 32)
